...
class RemotePredictRepository(private val predictAPI: PredictAPI) : PredictRepository {
    ...
    // dto로 변환후 예측 수행
    override suspend fun predict(isExercised: Boolean, authToken: AuthToken, basicInfo: BasicInfo, latestBloodPressure: BloodPressure?, latestSleepHour: SleepHour?): ApiResponse<PredictResponseDTO> {
        ...
        return predictAPI.predict(authToken.getHeaderAccessToken(), PredictRequestDTO(sex, age, systolic, diastolic, bmi, smoking, walk, totalSleep))
    }
}